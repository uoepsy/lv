...
Thanks to [Sewal Wright](https://www.jstor.org/stable/pdf/2527551.pdf?casa_token=3QF0ad2ZoBcAAAAA:MbEkDNNdoLZr1SXE4LrnK--qrhhsTXLgsRtcWre1UvWxiQiGNUl5vWytGp34XIxhAYMZJe-MbIcBnEwXSfX6MAONevz04-sMXpEDI3IaYKk6mMX46QvX), we can express the covariance between any two variables in the system as the sum of all *compound paths* between the two variables^[if we were wanting *correlations*, and we didn't have standardised variables, then these would have to be divided by the corresponding standard deviation estimates for the variables].  
...
The process is much easier with correlations, so in the examples below everything is standardised to have a variance of 1, meaning that covariance == correlation.  
...
just like CFA, the fit of these models can be assessed (when over identified) 

we can also do model comparisons, e.g., between the models in examples 2 and 3
