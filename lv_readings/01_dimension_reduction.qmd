...
# Dimension Reduction, Discovery, and Testing
...
5. How do we represent a person's standing on the dimensions?
    - Add up all the responses on each variable $\text{sum score} = y_{1i} + y_{2i} + y_{3i} +\, ... \, + y_{pi}$, and if you want a mean-score, we then divide by the number of variables: $\text{mean score} = \frac{y_{1i} + y_{2i} + y_{3i} +\, ... \, + y_{pi}}{p}$.
...
5. How do we represent a person's standing on the dimensions?
    - Extract PCA scores (these are a weighted sum of the scores on each variables: $\text{score}_{\text{component j}} = w_{1j}y_1 + w_{2j}y_2 + w_{3j}y_3 +\, ... \, + w_{pj}y_p$). See [PCA Walkthrough](02_pca.html#scores).  
