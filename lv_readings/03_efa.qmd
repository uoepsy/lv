...
Much like PCA (see [Chapter 2 #the-key-methods](02_dimension_reduction.html#the-key-methods){target="_blank"}), the scores from factor models will be taking into account the extent to which each individual observed variable might differently reflect the underlying factor. So this will contrast with naive scale scores which assume each variable is equally reflective of the construct.  
