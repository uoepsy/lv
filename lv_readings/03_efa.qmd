...
It's worth noting that exploratory factor analysis is a model-based approach to dimension reduction. This differs subtley from something like PCA which is more like a calculation or 're-expression'.  
...
The shift in factor analysis is that our "outcome" is going to be our covariance matrix. The "model" is going to be some function of our "factor loadings" (the associations between each item and the factor), and the "error" is going to be the 'how far off is the model from the outcome?'  

In the example below, we have three observed variables, meaning that we have a 3x3 covariance matrix. We are modelling this with just one factor, so our model is just 3 numbers (the loadings of each variable onto the single factor). We're using $\mathbf{\Lambda}$ to denote those 3 numbers, and our "model" is $\mathbf{\Lambda \Lambda'}$ (Don't worry too much about what this means, just note that a) it is just a function of $\mathbf{\Lambda}$ and b) it results in a 3x3 matrix that is *close* to (but not quite identical) our observed covariance matrix!  
...
\text{Outcome} &= \text{Model}\, +\, \text{Error}  \\
\quad \\
\text{Covariance Matrix} &= \text{Factor Loadings}^2 \,+\, \text{Uniqueness} \\
\quad \\
...
\end{align}
$$
Note that what is being captured by the model is the co-variance --- i.e, the "common" or "shared" variance. The 'leftover' bits in the error is the variance that is "unique" to each observed variable, and this will include some stuff that is specific to that variable, and some stuff that is just measurement error. 

Suppose our four variables were all questions about anxiety. For agreement with a statement like like "I struggle to say still and am often fidgeting", people will vary in their responses due to 1) their level of anxiety, 2) their general fidgeting behaviours that are separate from their anxiety, and 3) measurement error (responses of "agree" and "strongly agree" are very inexact measurements here!)  
...
We're not going to delve too much into factor extraction methods here, beyond noting that there are various different methods.  
...
Now think back to how we can scale a predictor in a linear model without actually changing the model: `lm(y ~ x)` is identical to `lm(y ~ scale(x))`. In a similar way, we can actually _transform_ the set of relationships between measured variables and dimensions, without really 'changing the model' (i.e., without changing what the model predicts).    
This idea is termed as "rotational indeterminacy" because there are infinitely many sets of numbers that can reproduce the same thing! So it's a little bit like 'scaling' in the sense that it is a transformation applied to the set of loadings.  

The reason for applying these transformations is because it can make the factors a whole lot more theoretically feasible. If we return to our 10 questions on tiredness where we extract 2 factors, note that we might have a fairly difficult time trying to make sense of what those factors represent. 
...
The first factor is associated fairly highly with all 10 variables (these numbers can range from -1 to 1). The second factor is more confusing - it's negatively associated with four questions on mental fatigue, and positively associated with physical fatigue. So we end up trying to conclude that the questions are capturing one underlying dimension of "general fatigue" and another confusing dimension that is sort of "being physically fatigued but not mentally fatigued (or vice versa)". It's not the clearest idea, especially as when we introduced these questions we actually had a clearer notion (i.e., some questions about mental fatigue, some questions about physical fatigue, and a couple of others)
...
One way to *see* this rotation is to take a little plot of the loadings for each variable on to each factor: @fig-biplot-none (this is only really possible here because we have just two factors). We can rotate the factor axes, so that the loadings on these new axes are generally higher on one axis and lower on the other, as in @fig-biplot-orth.  
...
p2 + labs(title="Orthogonal Rotation")
```
:::
::::

Note that these new set of loadings make it a lot easier to provide a meaningful interpretation of our factors - we have one factor that seems to represent mental fatigue + sleepiness, and another factor that seems to represent physical fatigue + sleepiness.  
The total variance explained by this rotated solution is just the same as our unrotated solution (this is the 0.365 in "Cumulative Var" that tells us that cumulatively, the two factors capture 36.5% of the total variance).  
...
Note that even when we are doing this rotation we are working under the constraint that the two factors are "orthogonal". We might be rotating but we are preserving the right angle between them, meaning that whether or not someone is low/high on the first factor is *unrelated* to their position on the second factor.  

Hopefully you are uncomfortable with this assumption --- why should we have to assume that someone's level of physical fatigue is uncorrelated with their level of mental fatigue? Surely we'd expect them to be correlated!?  

Well, we can do this, with what is known as an "oblique rotation". The idea is much the same as the "orthogonal rotation" above, but we don't have to keep the right angle between the factors:  
...
p3 + labs(title="Oblique Rotation")
```
:::
::::

What we get out now is a lovely clean structure where each variable is clearly linked to one factor and not as much to the other, and we have let the factors be correlated - so for someone who is high on factor 1 (now the 'physical fatigue' factor) we would also expect them to be high on factor 2 ('mental fatigue').  
```{r}
tired_fa3 <- fa(tiredq, nfactors=2, rotate = "oblimin", fm="ml")
print(tired_fa3$loadings, cutoff = .3)
```

In this situation, we also get out the correlation between the factors^[If you're a trigonometry fan (I'm not!), it's the cosine of the angle between the axes!]. We can extract the specific part using:
...
If we are thinking that there might be two or more factors that underly our scale of 'phone-addiction', then we would probably expect those two factors to be correlated (all the questions are about phone usage, after all). For that reason, we are going to use an **oblique rotation**^[additional point here is that even if these two factors are not correlated, then an oblique rotation could estimate that too.]  
...
```{r}
ph_efa1 <- fa(phdat, nfactors = 1, fm = "ml")
ph_efa2 <- fa(phdat, nfactors = 2, fm = "ml", rotate = "oblimin")
```
