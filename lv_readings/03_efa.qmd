...
The shift in factor analysis is that **our "outcome" is going to be a covariance matrix.**
This covariance matrix represents how the items that we've collected data for co-vary with one another.
You could think of covariance as how much you can predict about Item B when you already know Item A.
For example, if responses to Item B are generally high when responses to Item A are also high, then the two co-vary quite a bit.
On the other hand, if responses to Item B are all over the place when responses to Item A are high, then the two don't co-vary much at all.
