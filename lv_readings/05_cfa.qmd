...
This model means we estimate all those covariances between each pair of variables, and so we can *perfectly* reproduce any observed covariance matrix.
...
(And we can't actually compute whether this model is a good fit to the data, because we're estimating the exact same number of parameters as there are data points in the covariance matrix—more on this in the degrees of freedom section below!)
