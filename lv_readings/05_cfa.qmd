...
We are going to have to somehow give our latent factor a numeric scale, and one way to do this is to fix its variance to be 1 (we'll see more about this in a little bit). This helps somewhat, but not enough, we still have twelve things to be estimated. 

There is another easy way to make this kind of model overidentified. Specifically, CFA models will nearly always specify that the residual factor loadings are equal to 1.
This decision removes four of the unknown parameters, bringing the model down to only eight unknowns.
Because there are eight unknowns and ten knowns, the model is now overidentified—hurray!  
