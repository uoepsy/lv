...
## Making a model overidentified by defining residual factor loadings
...
Each of those observed items is associated with some unobserved measurement error, represented as $u_j$ for item $j$.
In CFA terms, we think of these measurement errors as unobserved factors (similar to the latent factor $F$), but we call these factors "residual factors".
Each residual factor has its own loading onto one of the observed variables as well as its own variance.
...
This model is underidentified because we only have ten knowns (four variances and six covariances) but thirteen parameters to estimate:
...
But there is a common way to make this kind of model overidentified.
Specifically, CFA models will nearly always specify that the residual factor loadings are equal to 1.
This decision removes four of the unknown parameters, bringing the model down to only nine unknowns.
Because there are nine unknowns and ten knowns, the model is now overidentified—hurray!

@fig-model-overidentified shows two ways of representing this overidentified model.
...
So if you see diagrams like the ones in Panel B, you know that those are models in which the residual factor loadings have been set to 1.

```{r}
#| echo: false
#| label: fig-model-overidentified
#| fig-cap: "A four item factor structure, shown two ways: one explicitly showing the residual factor loadings set to 1, one simpler version in which this has been assumed. The model is now overspecified because there are more known values (ten) than unknown values (nine)."
...
**Convention:** if $\textrm{RMSEA} < .05$, $\textrm{SRMR} < .08$, $\textrm{TLI} > .95$ and $\textrm{CFI} > .95$ then the model fits well.  
...
We can also quickly get out estimates that are standardised (or 'standardized' if you're North American!) by simply asking for them in the `summary()`, writing `standardized = TRUE`. The result will output extra columns next to the parameter estimates.  
