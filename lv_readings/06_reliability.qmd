...
Now let's expand from thinking about Dougal specifically to thinking about our proposed measurement process for weighing dogs in general. 

We have our "instrument" for measuring a dog's weights (our bathroom scales). We can imagine all the measurements we might use this instrument for. For any dog we might use these scales to weigh, the observed number we see on the scales is going to be driven by 1) that dog's *true* weight, and 2) a little bit of error.  
Just like all the true weights of all our dogs have a variance (some are big dogs that weigh a lot, some a little dogs that weigh less etc.), the errors that are contributing to our measurements also have a variance (some readings are a bit higher than truth, some are a bit lower).   
...
$$
Let's take these to the extreme: 

If my measurement instrument is incredibly good in weighing dogs, then the variance in the errors is much much smaller (i.e., all measurements are within 0.1kg of the true score). If my measurement instrument is incredibly bad at weighing dogs, then the variance in errors is much bigger (sometimes when I weigh a dog that is 'in truth' 23.814346....kg, I will get a score of 15kg, sometimes I will get a score 28kg etc).  
...
A. I am measuring the weight of dogs, and my scales are a bit off each time - there is a little bit of error. The errors have a variance of 1kg
B. I am measuring the weight of dogs, and my scales are *wayy* off each time - there is a massive amount of error. The errors have a variance of 100kg
C. I am measuring the weight of elephants, and my scales are a little bit off each time - there is a little bit of error. The errors have a variance of 100kg

The size of the error variance (1kg or 100kg) doesn't determine how much "off" (a little bit off vs waayy off) we consider the measurements to be. What is important is how big this error variance is **relative to** the true variance of the things being measured.  
