...
![](images/montoya.jpg){width="250px"}
...
Both "face validity" and "content validity" are not easily assessed, and so ultimately it comes down to just reading the items and thinking about the theory of what we actually _mean_ when by some construct like "anxiety".  

However, there are forms of validity for which we can use stats to (albeit indirectly) inform our trust in the measurement, and this is **"convergent"** and **"discriminant"** validity. Loosely speaking, this is asking whether the measure correlates with measures of other things that we would (theoretically) expect it to correlate with, and whether it _doesn't correlate_ with things we expect it _not_ to correlate with.  

These ideas are quite interesting, especially when we consider the utility of a measure in the grand scheme of things. If I have a lovely new measure called "The Edinburgh Anxiety Scale"^[Couldn't bring myself to call it "the Josiah King Anxiety Scale"! :D], then I would hope that it a) correlates high with other established measures of anxiety (like the GAD-7) and b) correlates only weak/moderately with a measure of depression (like the Beck depression inventory). 

The key thing here is that we want to ensure that two measures of the same thing will be highly correlated, but also that measures of different things are not too highly correlated. This latter idea is important because we want to ensure that we're not accidentally measuring something else! It leads back to the idea of the ["jingle-jangle fallacies"](https://en.wikipedia.org/wiki/Jingle-jangle_fallacies){target="_blank"} - if I create a measure of "grit" I would like to be confident that this is something different from "resilience" or "perserverance" etc.  
...
The way we would assess these is to build a "nomological net" - we would measure people on the "Edinburgh Anxiety Scale" and also on the GAD7 and also on Beck's Depression Inventory. We would then fit a CFA model, and examine the correlations between the factors:  
...
_How much does my duck look like 'the perfect duck'? If I am a duck, will there by duck eggs in my future?_  
...
Often, the 'standard' is some future event, and so this gets termed 'predictive validity'. A common example is that in the USA many colleges have entrance exams, and so we might reasonably ask if scores on the exam predict their performance in their first year of college. Similarly, we might ask if scores on a driving test predict lower accident risk.  
...
The really useful thing about diagrams like these is that they represent theoretical models that can be used to *imply* what we would expect to see in our observed variables (we saw this idea in [CFA](05_cfa.html)). One crucial thing that this specific diagrammatic model of measurement implies is that the variance we would see in our observed scores is going to be the variance in the true scores plus the variance in the errors.    
...
There are two further implications by thinking of our measurements as it is drawn in @fig-ctt2.  
...
2. Error has mean zero. This is just like we often assume with errors - they are just randomness deviations, meaning they average out at zero. When it gets to diagrams and covariance-based models (like CFA etc), we don't really talk much about the means anymore. In these diagrams we introduce means as a constant fixed number, in **triangles** (if squares are for observed variables, circles are unobserved/latent variables, then we need a new shape). So more explicitly our diagram would look like @fig-cttmz, where the errors have a mean zero as indicated by the triangle going into the "Error". The triangle going into the "Truth" represents the average true value, $\mu$. The lack of an arrow going into the observed score is telling us that there is no systematic bias (i.e., in our weighing scales example, if they always weight 5kg heavier, we would have a triangle in the 5 going into the score).  
...
When these assumptions do not hold, we end up with the measurement tool having different levels of reliability for different levels of truth - e.g., a reliable means of weighing dogs between 15-30kg but not a reliable means of weighing dogs between 5-10kg. Or, if the errors do not have mean zero then this could be systematic bias, or could be due to something skewing our errors, meaning we might have some measurements being much further off than others (in the right hand plot below, if we took all the errors - the red lines - and plotted them, it would look very skewed, and have a mean of ~0.8).  
...
If reliability is a **proportion**, this means it can range from 0 (all of the variance in scores is error) to 1 (none of it is). There is no defined cut-off for what is "reliable enough" as a measurement tool. In a lot of social science, people seem to want at realibility to be >0.7.  

However, while this way of thinking about reliability might seem nice and intuitive, how on earth do we actually _quantify_ it? With a single set of scores, we don't have enough information... We are essentially saying "observed scores = unobserved true values + unobserved errors". This is a bit like being given a math problem of "23.5 = T + E, please solve" --- it's impossible! So while we might know that $Var(\text{Observed Scores})$=$Var(\text{True Scores})$+$Var(\text{Errors})$, with just one set of scores we can't figure out how much of the observed variance is true variance vs error variance. 

Things get a lot better when we have multiple assessments. This is because we have two scores, and two errors, but only one truth. And we have a certain set of assumptions (some of which we can see in the diagram):  
...
For all of these, the simplest idea is just to use $cor_{score_1,score_2}$ as our estimate of reliability. 
However, A standard Pearson's correlation (i.e., `cor(score1, score2)`) will simply look for a linear relationship, but this means that if we had 2 sets of scores: $scores1= \{2,3,5\}$ and $scores2= \{20,30,50\}$ then we would have perfect reliability (because they are perfectly correlated). 

A more common approach in psychometrics is to use the **Intraclass Correlation Coefficient (ICC)**. Because this works on partitioning up variance between and within the observed units, this is more like assessing *agreement* between tests. Variance in average scores *between* the units in our sample can be taken as an estimate of "how much units truly vary", and the variance in scores *within* the units can be seen as the error variance.  
...
Unfortunately, it spits out a whole bunch of different *types* of ICC, which can get a bit confusing, so we end up having to go into the weeds a bit.  

From the help pages of the function, it tells us that these are: 
...
Then for each of these we have the equivalent for if there are more than one rating per judge-target pair.  
...
While the above forms of reliability are concerned with each test administration providing a single numeric score, which is then assessed over different times/versions/raters, very often we are faced with a single administration of a psychometric test providing multiple numeric scores, such as when we ask a bunch of questions.  

This happens all the time in psychology, and in order to quantify reliability, we are looking at it all _within_ a single administration of our measurement tool. For some reason, this typically gets termed "internal *consistency*".  
...
Just as above, the way we try to quantify reliability in these cases is by figuring out what bit of all the variance in responses could be due to "true" variance in an underlying construct, vs what might just be error. Far and away the most famous of these is "**coefficient alpha ($\alpha$)**" (often called "Cronbach's alpha"^[Lee Cronbach has said we shouldn't it call 'Cronbach's Alpha' because he didn't come up with the idea]), which has a pretty unsightly formula, but conceptually uses covariance between items as a measure of "true variance", and takes this as a proportion of the variance in total scores across the items.  

By using the average covariance to capture "true variance", what we are implicitly assuming is that each of those covariances (so all of the items) are equally important. Sounds kind of familiar? It's just the same assumption we had when we calculated a sum score, or a mean score - hence why our denominator is the variance in a sum score!  
...
:::rtip
In R, we can calculate McDonald's Omega with the `omega()`, or `omegaFromSem()` functions from the **psych** package.  

The latter will require us to fit the model with `cfa()` first, the former will require us to specify how many factors there are.  
...
# The Impact of (Un)Reliability  

All these different ways to estimate the reliability of measures, and for what purpose? At the very fundamental level, it means that whenever we go out into the world and use our measurement tool, we have an idea of "how far off" the observed score is likely to be. It tells us how much faith we should have in our measurements!  One handy way of conceptualising this is to turn reliability into the **"standard error of measurement"**, which can be used to get a range for the true value behind any individual observed score.
...
If nothing else, take away just these points:  
