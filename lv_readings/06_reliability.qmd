...
Now let's expand from thinking about Dougal specifically to thinking about the measurement process of weighing dogs.  

We have our "instrument" for measuring dog's weights (our bathroom scales). We can imagine all the measurements we might use this instrument for.  

For any dog we might use these scales to weigh, the observed number we see on the scales is going to be driven by 1) that dog's *true* weight, and 2) a little bit of error.  

Just like all the true weights of all our dogs have a variance (some weigh a lot, some weigh less etc.,), the errors that are contributing to our measurements also have a variance (some readings are a bit higher than truth, some are a bit lower).   

If we add these to our diagram, we simply add curved lines to our "Truth" and "Error" variables: 
```{r}
#| label: fig-ctt2
#| echo: false
#| fig-width: 12
...
These diagrams are theoretical models that can be used to *imply* what we would expect to see in our observed variables (we saw this idea in CFA). One crucial thing that this model implies that the variance we would see in our observed scores is going to be the variance in the true scores plus the variance in the errors.    

Let's take these to the extreme: 

If my measurement instrument is incredibly good in weighing dogs, then the variance in the errors is much much smaller (i.e., all measurements are within 0.1kg of the true score). 
If my measurement instrument is incredibly bad at weighing dogs, then the variance in errors is much bigger (sometimes when I weigh Dougal I get 15kg, sometimes I get 28kg etc).  

So reliability is about t
