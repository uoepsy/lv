...
A correlation matrix is a square matrix (i.e. same number of columns as rows), and it is symmetric on the diagonals. One way in which we could create this sort of matrix is to take the product of some vector $\mathbf{f}$ with $\mathbf{f'}$ (f "transposed").^[this is just how you might "square" a vector. To multiple two vectors you take each entry of the first and multiply it with each entry of the second TODO]  
For instance:  
$$
\begin{equation*}
\mathbf{f} = 
...
The one thing about a correlation matrix is that the diagonal values are all 1 (the correlation of a variable with itself is 1). So let us suppose we have a correlation matrix (lets call it **R**) that looks like this:  
$$
\begin{equation*}
\mathbf{R} = 
\begin{bmatrix}
...
PCA is about trying to get at a set of vectors which could generate the correlation matrix **R**.  

We express $\mathbf{R = CC'}$, where $\mathbf{C}$ are our principal components.  
